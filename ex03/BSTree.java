...
public class BSTree {
    ...
    private void insert(String s,Node node){
        ...
        if(node.hasLeft() && s.compareTo(node.getElement()) < 0) insert(s, node.getLeft());
        
        else if(node.hasRight() && s.compareTo(node.getElement()) > 0) insert(s, node.getRight());
    ...
    private void bfs(Node node){  
        ...
        while(Q.size() != 0)
        {
            ...
            if(temp.hasLeft()) Q.add(temp.getLeft());
            if(temp.hasRight()) Q.add(temp.getRight());
    ...
    private void dfs(Node node){
        ...
        while(Q.size() != 0)
        {
            ...
            if(temp.hasLeft()) Q.push(temp.getLeft());
            if(temp.hasRight()) Q.push(temp.getRight());
