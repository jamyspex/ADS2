...
public class StringList {
    ...
    public boolean equal(StringList l){
        ...
        Node cursorArg = l.getHead();
        Node cursorLocal = getHead();

        while(cursorLocal.getNext() != null)
        {
            if(!cursorLocal.getElement().equals(cursorArg.getElement()))
            {
                return false;
            }
            cursorLocal = cursorLocal.getNext();
            cursorArg = cursorArg.getNext();
        }
    ...
    public StringList reverse(){
        ...
        StringList reversed = new StringList();
        Node cursor = getHead();
        
        for(int i = 0;  i < size(); i++)
        {
           
            reversed.addFront(cursor.getElement());
